"""GGUF checkpoint reader and metadata-first MoE topology adapters.
...
_QWEN35MOE_ARCHITECTURE = "qwen35moe"
DEFAULT_REGISTRY_PRIORITY = 200
QWEN35MOE_REGISTRY_PRIORITY = 190
...
class Gemma4GgufBackend:
    ...
    def supports(self, signature: BackendSignature) -> bool:
        ...
        return architecture == _GEMMA4_ARCHITECTURE
    ...
    def validate_bundle(self, bundle: LoadedBackendBundle) -> None:
        ...
        if bundle.config.get("general.architecture") != _GEMMA4_ARCHITECTURE:
...
class Qwen35MoeGgufBackend:
    """Backend adapter for Qwen3.5-MoE GGUF checkpoints."""
    ...
    def supports(self, signature: BackendSignature) -> bool:
        ...
        if signature.metadata.get("format") != "gguf":
            return False
        architecture = signature.architecture or signature.model_type
        return architecture == _QWEN35MOE_ARCHITECTURE
    ...
    def load(
        ...
        *,
        ...
        """Load a local Qwen3.5-MoE GGUF file into a lightweight bundle."""

        if signature.source_path is None:
            raise TopologyMismatchError("Qwen3.5-MoE GGUF backend requires source_path")
        checkpoint = open_local_gguf_checkpoint(signature.source_path)
        bundle = LoadedBackendBundle(
            backend_name=self.name,
            model_handle=ModelHandle(
    ...
    def iter_layers(self, bundle: LoadedBackendBundle) -> tuple[LayerTopology, ...]:
        """Yield deterministic Qwen3.5-MoE GGUF layer topology metadata."""
    ...
    def extract_topology(self, bundle: LoadedBackendBundle) -> tuple[LayerTopology, ...]:
        ...
        config = bundle.config
        block_count = _required_int(config, "qwen35moe.block_count", model_id=bundle.model_handle.model_id)
        hidden_size = _required_int(
            config,
            "qwen35moe.embedding_length",
            model_id=bundle.model_handle.model_id,
        )
        expert_count = _required_int(
            config,
        ...
        expert_width = _required_int(
            config,
            "qwen35moe.expert_feed_forward_length",
            model_id=bundle.model_handle.model_id,
        )
        shared_width = _optional_int(config, "qwen35moe.expert_shared_feed_forward_length")
        full_attention_interval = _optional_int(config, "qwen35moe.full_attention_interval")
        layers: list[LayerTopology] = []
        for layer_index in range(block_count):
            is_full_attention = (
                full_attention_interval is not None
                and full_attention_interval > 0
                and (layer_index + 1) % full_attention_interval == 0
            )
            layers.append(
                LayerTopology(
                    layer_index=layer_index,
                    layer_name=f"blk.{layer_index}",
                    layer_type="qwen35moe_gguf_moe",
                    expert_count=expert_count,
                    top_k=top_k,
                    hidden_size=hidden_size,
                    moe_intermediate_size=expert_width,
                    expert_dim=expert_width,
                    ffn_in_features=hidden_size,
                    ffn_out_features=hidden_size,
                    layer_ref=f"layer_{layer_index}",
                    module_paths=_qwen35moe_layer_paths(layer_index),
                    metadata={
                        "format": "gguf",
                        "attention_type": "full_attention" if is_full_attention else "linear_attention",
                        "has_ssm": not is_full_attention,
                        "shared_expert_feed_forward_length": shared_width,
                    },
                )
            )
        return tuple(layers)
    ...
    def extract_router_state(
        ...
        *,
        ...
        """Extract router metadata for a single Qwen3.5-MoE GGUF layer."""

        tensor_index = _tensor_index(bundle)
        router_key = layer.module_paths["router_proj"]
        shared_router_key = layer.module_paths["shared_expert_router"]
        router = _required_tensor(tensor_index, router_key, bundle=bundle, layer=layer)
        shared_router = _required_tensor(tensor_index, shared_router_key, bundle=bundle, layer=layer)
        return RouterState(
            layer_index=layer.layer_index,
            num_experts=layer.expert_count,
            top_k=layer.top_k,
            ...
            top_k_indices_shape=(1, layer.top_k),
            top_k_weights_shape=(1, layer.top_k),
            projection_shape=router.data_shape,
            per_expert_scale_shape=None,
            has_router_probabilities=True,
            route_scale_present=False,
            metadata={
                "router_proj_key": router_key,
                "shared_expert_router_key": shared_router_key,
                "shared_expert_router_shape": "x".join(str(item) for item in shared_router.data_shape),
                "format": "gguf",
            },
        )
    ...
    def extract_expert_state(
        ...
        *,
        ...
        """Return per-layer expert and shared-expert tensor descriptors."""
    ...
    def validate_bundle(self, bundle: LoadedBackendBundle) -> None:
        """Validate required Qwen3.5-MoE GGUF metadata and layer tensors."""

        if bundle.config.get("general.architecture") != _QWEN35MOE_ARCHITECTURE:
            raise BackendMismatchError(
                "Qwen3.5-MoE GGUF backend requires general.architecture=qwen35moe",
                model_id=bundle.model_handle.model_id,
                backend_name=self.name,
            )
        for layer in self.extract_topology(bundle):
    ...
    def validate_layer(
        ...
        *,
        ...
        """Validate Qwen3.5-MoE GGUF tensor presence and router shapes for one layer."""

        tensor_index = _tensor_index(bundle)
        for role, tensor_key in layer.module_paths.items():
            _required_tensor(tensor_index, tensor_key, bundle=bundle, layer=layer, role=role)

        router = tensor_index[layer.module_paths["router_proj"]]
        expected_router_data_shape = (layer.expert_count, layer.hidden_size)
        if router.data_shape != expected_router_data_shape:
            raise TopologyMismatchError(
                "Qwen3.5-MoE GGUF router projection shape mismatch",
                model_id=bundle.model_handle.model_id,
                layer_index=layer.layer_index,
                tensor_key=router.tensor_key,
                expected_shape=expected_router_data_shape,
                actual_shape=router.data_shape,
            )
        if router.tensor_type != "F32":
            raise ShapeInvariantViolationError(
                "Qwen3.5-MoE GGUF router projection must be F32 for static scan",
                model_id=bundle.model_handle.model_id,
                layer_index=layer.layer_index,
                tensor_key=router.tensor_key,
                details={"tensor_type": router.tensor_type},
            )
        _validate_qwen35moe_expert_shapes(bundle=bundle, layer=layer, tensor_index=tensor_index)
        if router_state is not None and router_state.projection_shape != router.data_shape:
            raise TopologyMismatchError(
                "Qwen3.5-MoE GGUF router_state projection shape mismatch",
                model_id=bundle.model_handle.model_id,
                layer_index=layer.layer_index,
            )
    ...
    def resolve_router_module(self, bundle: LoadedBackendBundle, *, layer: LayerTopology) -> object:
        ...
        raise TopologyMismatchError(
            "Qwen3.5-MoE GGUF backend does not expose runtime router modules for bench",
            model_id=bundle.model_handle.model_id,
            layer_index=layer.layer_index,
        )
...
def qwen35moe_registry_entry() -> tuple[Qwen35MoeGgufBackend, int]:
    """Return the default registry entry for GGUF Qwen3.5-MoE checkpoints."""

    return Qwen35MoeGgufBackend(), QWEN35MOE_REGISTRY_PRIORITY
...
def _qwen35moe_layer_paths(layer_index: int) -> dict[str, str]:
    prefix = f"blk.{layer_index}"
    return {
        "router_proj": f"{prefix}.ffn_gate_inp.weight",
        "shared_expert_router": f"{prefix}.ffn_gate_inp_shexp.weight",
        "experts_gate_proj": f"{prefix}.ffn_gate_exps.weight",
        "experts_up_proj": f"{prefix}.ffn_up_exps.weight",
        "experts_down_proj": f"{prefix}.ffn_down_exps.weight",
        "shared_expert_gate_proj": f"{prefix}.ffn_gate_shexp.weight",
        "shared_expert_up_proj": f"{prefix}.ffn_up_shexp.weight",
        "shared_expert_down_proj": f"{prefix}.ffn_down_shexp.weight",
    }


def _validate_qwen35moe_expert_shapes(
    *,
    bundle: LoadedBackendBundle,
    layer: LayerTopology,
    ...
    if layer.moe_intermediate_size is None:
        raise TopologyMismatchError(
            "Qwen3.5-MoE GGUF layer must include moe_intermediate_size",
            model_id=bundle.model_handle.model_id,
            layer_index=layer.layer_index,
        )
    expected: Mapping[str, tuple[int, ...]] = {
        "experts_gate_proj": (layer.hidden_size, layer.moe_intermediate_size, layer.expert_count),
        "experts_up_proj": (layer.hidden_size, layer.moe_intermediate_size, layer.expert_count),
        "experts_down_proj": (layer.moe_intermediate_size, layer.hidden_size, layer.expert_count),
        "shared_expert_gate_proj": (layer.hidden_size, layer.moe_intermediate_size),
        "shared_expert_up_proj": (layer.hidden_size, layer.moe_intermediate_size),
        "shared_expert_down_proj": (layer.moe_intermediate_size, layer.hidden_size),
        "shared_expert_router": (layer.hidden_size,),
    }
    for role, expected_shape in expected.items():
        tensor = tensor_index[layer.module_paths[role]]
        if tensor.shape != expected_shape:
            raise ShapeInvariantViolationError(
                "Qwen3.5-MoE GGUF tensor shape mismatch",
                model_id=bundle.model_handle.model_id,
                layer_index=layer.layer_index,
                tensor_key=tensor.tensor_key,
                expected_shape=expected_shape,
                actual_shape=tensor.shape,
                details={"tensor_role": role},
            )
