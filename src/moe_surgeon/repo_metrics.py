...
def load_verify_config(root_path: Path) -> VerifyConfig:
    ...
    if not isinstance(metrics_config, dict):
        raise MetricsConfigurationError("Missing repoMetricsConfig mapping in .supervisor/project.json")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        help="Optional path for the JSON metrics artifact.",
    )
    parser.add_argument(
        "--timeout-seconds",
        type=int,
    ...
    output_path = _resolve_output_path(root_path, args.task_id, args.output)
    write_report(final_report, output_path)
    if args.task_id:
        append_task_log(root_path, args.task_id, final_report, artifact_path=output_path)
